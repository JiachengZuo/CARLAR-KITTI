...
class World(object):
    ...
    def restart(self):
        ...
        if self.player is not None:
            ...
            spawn_point.location.x = 41.8
            spawn_point.location.y = -76.7
        ...
        while self.player is None:
            ...
            location = carla.Location(5.2, 96.1, 2.0)
...
class KeyboardControl(object):
    ...
    def __init__(self, world, sim_world, start_in_autopilot):
        self._autopilot_enabled = start_in_autopilot
        self._ackermann_enabled = False
        self._ackermann_reverse = 1
        if isinstance(world.player, carla.Vehicle):
            self.origin_settings = sim_world.get_settings()
    ...
    def parse_events(self, client, world, clock, sync_mode):
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                world.apply_settings(self.origin_settings)
                return True
...
# 最里侧90,中间270
def vehicle_npc_create(sim_world):
        vehicle_list = []
        blueprint_library = sim_world.get_blueprint_library()
        # car_1
        for i in range(23):
            location = carla.Location(107.33 - i * 6, -85.9, 0.5)
            rotation = carla.Rotation(0, 270 ,0)
            spawn_point = carla.libcarla.Transform(location,rotation)
        ...
        for i in range(25):
            location = carla.Location(112.6 - i * 6, -98.7, 0.5)
        ...
        for i in range(3):
            location = carla.Location(69.6 - i * 10, -63.8, 0.5)
            rotation = carla.Rotation(0, 270 ,0)
            spawn_point = carla.libcarla.Transform(location,rotation)
            vehicle_bp = blueprint_library.find('vehicle.ford.crown')
            vehicle_bp.set_attribute('color', '0, 0, 0')  
            # spawn the vehicle
            vehicle = sim_world.spawn_actor(vehicle_bp, spawn_point)
            vehicle_list.append(vehicle)
        ...
        for i in range(3):
            location = carla.Location(32 - i * 10, -63.8, 0.5)
            rotation = carla.Rotation(0, 270 ,0)
            spawn_point = carla.libcarla.Transform(location,rotation)
            vehicle_bp = blueprint_library.find('vehicle.ford.crown')
            vehicle_bp.set_attribute('color', '0, 0, 0')  
        ...
        for i in range(3):
            location = carla.Location(14 - i * 10, -63.8, 0.5)
            rotation = carla.Rotation(0, 270 ,0)
            spawn_point = carla.libcarla.Transform(location,rotation)
            vehicle_bp = blueprint_library.find('vehicle.ford.crown')
            vehicle_bp.set_attribute('color', '0, 0, 0')  
...
def game_loop(args):
    ...
    try:
        ...
        controller = KeyboardControl(world, sim_world, args.autopilot)

        model.sensor_listen()
        step = 0
        STEP = cfg["SAVE_CONFIG"]["STEP"]
        # vehicle_list = None
        # vehicle_list = vehicle_npc_create(sim_world)
